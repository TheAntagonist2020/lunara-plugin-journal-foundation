<?php
/**
 * Executable stabilization contracts for Journal Foundation 1.2.5.
 *
...
contract_contains( $main, 'Version: 1.2.5', 'Plugin header must report 1.2.5.' );
contract_contains( $main, "const VERSION             = '1.2.5';", 'Runtime Foundation version must be 1.2.5.' );
contract_contains( $readme, 'Version: 1.2.5', 'README must report Foundation 1.2.5.' );
...
foreach ( array( 'production' => $production_openapi, 'bridge' => $bridge_openapi, 'staging' => $staging_openapi ) as $label => $openapi_release ) {
    contract_contains( $openapi_release, '"version": "1.2.5"', ucfirst( $label ) . ' OpenAPI release version must report 1.2.5.' );
}
